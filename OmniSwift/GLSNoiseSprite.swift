...
public class GLSNoiseSprite: GLSNode, DoubleBuffered {
    ...
    public init(size:CGSize, texture:CCTexture?, color:SCVector3, noiseColor:SCVector4, tiles:Int) {
    ...
    public func applyNoiseVertices() {
        ...
        for iii in 0..<TexturedQuad.verticesPerQuad {
            ...
            let point = texture * CGPoint(x: self.contentSize.width, y: self.contentSize.height)
            ...
            if let vTex = self.noiseTexture {
                ...
                let sizeP = CGPoint(x: vTex.frame.size.width, y: vTex.frame.size.height)
