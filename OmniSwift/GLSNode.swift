...
public class GLSNode: NSObject {
    ...
    public var projection:SCMatrix4 {
        get {
            if let proj = self.internalProjection ?? self.universalRenderer?.projection ?? self.recursiveRenderer?.projection {
                return proj
            } else {
//                print("Invalid Projection!")
    ...
    *  assign the singleton object to this property.
    *  When nodes are added as children to that node, the parent
    *  will set it for you and appropriately insert as needed.
    *  When the child is removed from its parent, the parent
    *  removes it appropriately.
    */
    public weak var universalRenderer:GLSUniversalRenderer? = nil
