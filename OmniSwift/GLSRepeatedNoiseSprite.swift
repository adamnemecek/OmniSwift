...
//        let positionRect = CGRect(size: size)
//        let textureRect = CGRect(square: 1.0)
        /*
    ...
    public func noiseSizeChanged() {
        ...
        let noisePoint = self.noiseSize.getCGPoint()
        let percent = (noisePoint - 1.0) / noisePoint

        let noiseRect = CGRect(size: self.noiseSize)
        let positionRect = CGRect(size: self.contentSize)
        let textureRect = self.shadeTexture?.frame ?? CGRect(square: 1.0)
